...
action_runner は実行環境に `ACTION_RUNNER_LOCAL=true` を自動設定します。`if:` 条件でローカル実行時のみステップをスキップ/実行できます：
...
GitHub Actions では `ACTION_RUNNER_LOCAL` は未設定なので、`!env.ACTION_RUNNER_LOCAL` は `true` に評価され全ステップが通常通り実行されます。
