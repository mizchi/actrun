...
- CLI run store の最小対応 (`--run-root`, `_build/action_runner/runs/<run-id>/run.json`, `jobs.json`, `artifacts.json`, `caches.json`, `tasks/*.stdout.log|stderr.log|summary.md`, jobs/artifacts/caches index, timestamps, `run list`, `run view`, `run watch`, `run logs`, `run download`, `artifact list`, `artifact download`, `cache list`)
